...
class Conversation {
  ...
  getPromptArrayUnproccessed() {
    ...
    for (
      let i = tvmjsGlobalEnv.workerHistoryMsg.length - 2;
      i < tvmjsGlobalEnv.workerHistoryMsg.length;
      ++i
    ) {
      const item = tvmjsGlobalEnv.workerHistoryMsg[i];
      const role = item[0];
  ...
  switchConversation(message: [string, string][]) {
    tvmjsGlobalEnv.workerHistoryMsg = message;
    this.convId = tvmjsGlobalEnv.covId;
  }
}
...
function defaultConversation(maxWindowLength = 2048) {
  return new Conversation({
    system:
      "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions. Follow the user's instructions carefully. Respond using markdown.",
...
class LLMChatPipeline {
  ...
  constructor(
    tvm: any,
    tokenizer: void,
    cacheMetadata: { ParamSize: any } | any,
    config: { maxWindowLength: any; maxGenLength: any; meanGenLength: any },
  ) {
  ...
  #forward(inputs: { shape: number[] }, curPos: any) {
    this.tvm.beginScope();
    var retValue;
  ...
  #updateLogitsOnCPU(logits: { shape: any[]; dtype: any }) {
    if (this.logitsOnCPU == undefined) {
  ...
  async generate(
    inputPrompt: any,
    callbackUpdateResponse: (arg0: number, arg1: string) => void,
  ) {
    ...
    tvmjsGlobalEnv.workerHistoryMsg[
      tvmjsGlobalEnv.workerHistoryMsg.length - 1
    ][1] = outputPrompt;
    return outputPrompt;
  }
...
class LLMChatInstance {
   ...
   */
  async #asyncInitTVM(wasmUrl: RequestInfo, cacheUrl: any) {
    ...
    this.tvm = tvm;
    const initProgressCallback = (report: { text: any }) => {
  ...
  appendMessage(kind: string, text: string, ifFinish?: boolean) {
    if (kind == 'initing') {
  ...
  updateLastMessage(type: string, text: string, ifFinish?: boolean) {
    if (type == 'initing') {
      text = `[System Initalize] ${text}`;
    }
    globalThis.postMessage({
  ...
  async respondTestMessage(repeat: number) {
  ...
  async generate() {
    ...
    const callbackUpdateResponse = (step: any, msg: string) => {
      if (msg.endsWith('##')) {
